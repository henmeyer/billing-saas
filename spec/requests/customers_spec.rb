...
RSpec.describe "Customers", type: :request do
  ...
  describe "GET /customers/:id" do
    context "with multiple active subscriptions" do
      ...
      let(:plan_a) { create(:plan, account: account, name: "Plan A", price_cents: 10_000) }
      let(:plan_b) { create(:plan, account: account, name: "Plan B", price_cents: 20_000) }
      ...
      it "includes value breakdown in each subscription" do
        ...
        expect(sub_with_extras[:period_amount_cents]).to eq(15_000)
        expect(sub_with_extras[:period_base_cents]).to eq(10_000)
    ...
    context "with subscription including extras" do
      ...
      let(:plan) { create(:plan, account: account, name: "Pro Plan", price_cents: 19_700) }
      ...
      it "shows has_extras as false when no extras exist" do
        ...
        expect(sub_data[:period_amount_cents]).to eq(19_700)
      end
      ...
      it "shows has_extras as true when extras exist" do
        ...
        expect(sub_data[:period_amount_cents]).to eq(25_000)
        expect(sub_data[:period_base_cents]).to eq(19_700)
      end
    end
